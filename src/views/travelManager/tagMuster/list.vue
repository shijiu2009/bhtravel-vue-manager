  ...
  <div>
    ...
    <div class="data_list">
      ...
      >
        ...
        <el-table-column prop="code" label="别名" sortable align="center" show-overflow-tooltip></el-table-column>
    ...
    <Detailed :content="tagMusterDetailed" ref="detailedMound"></Detailed>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    openDeleteWarning: function (ids) {
      this.$confirm("此操作将永久删除该数据么, 是否继续?", "提示", {
        ...
        .then(() => {
          //在列表中删除数据
          return tagMusterApi.deleteTagMuster({ ids: ids });
        })
        .then(() => {
          //删除成功刷新界面数据
          this.getTagMusters();
    ...
    //打开查看页面
    detailed: function (i) {
      this.$refs.detailedMound.updataComtent(this.tagMusterList[i]);
    },
    // 多选操作
    ...
    getTagMuster: function () {
      ...
      tagMusterApi
        ...
        .then((result) => {
          ...
          this.tagMusterList = result.rows;
