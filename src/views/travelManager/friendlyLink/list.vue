<template>
  <div>
    ...
    <Detailed :content="friendlyLinkDetailed" ref="detailedMound"></Detailed>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    openDeleteWarning: function (index, id) {
      this.$confirm("此操作将永久删除该数据么, 是否继续?", "提示", {
        ...
        .then(() => {
          //在列表中删除数据
          return friendlyLinkApi.deletefriendlyLink({ ids: id });
        })
        .then(() => {
    ...
    editLinkBtn: function (flow, id) {
      ...
      this.setTagsList("/addOrEditFriendlyLink");
    },
    //跳转查看页面
    detailed: function (i) {
      this.$refs.detailedMound.updataComtent(this.friendlyLinkList[i]);
    },
    // 多选操作
    ...
    getFriendlyLink: function () {
      ...
      friendlyLinkApi
        ...
        .then((result) => {
          ...
          this.friendlyLinkList = result.rows;
