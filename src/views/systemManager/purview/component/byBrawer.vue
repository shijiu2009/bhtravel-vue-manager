        ...
        <el-table :data="purViewList.slice((page.page-1)*page.rows,page.page*page.rows)" border           ref="multipleTable"
          ...
          <el-table-column width="60" align="center">
            <template slot-scope="scope">
              <el-radio v-model="pruViewInfo.pid" :label="scope.row.pid">
                <span></span>
              </el-radio>
            </template>
          </el-table-column>
          <el-table-column type="index" width="50" align="center" label="序号" sortable></el-table-column>
          <el-table-column property="pname" label="标题"></el-table-column>
          <el-table-column property="byid" label="父级id"></el-table-column>
          <el-table-column property="pid" label="pid"></el-table-column>
        </el-table>
      </div>
      <!-- 分页 -->
...
export default {
  ...
  methods: {
    getByPurViews: function () {
      this.loading = true;
      //添加参数
      this.page.spid = -1;
      purviewApi
        .getByPurViews(this.page)
        .then((result) => {
          this.loading = false;
          if (result.status) {
            this.purViewList = result.data;
    ...
    //确定按钮事件
    confirmBtn: function () {
      this.$parent.getByDrawer(this.pruViewInfo);
      this.isOpen = !this.isOpen;
    },
    //点击分页按钮
    ...
    rowClick: function (row) {
      this.pruViewInfo.pid = row.pid;
      this.pruViewInfo.pname = row.pname;
    },
  },
  created() {
    this.getByPurViews();
