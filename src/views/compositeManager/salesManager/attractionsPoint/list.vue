  ...
  methods: {
    ...
    openDeleteWarning: function (index, id) {
      this.$confirm("此操作将永久删除该数据么, 是否继续?", "提示", {
        ...
        .then(() => {
          //在列表中删除数据
          return attractionsPointApi.deleted({ ids: id });
        })
        .then(() => {
          this.getList();
          this.$message({
    ...
    editLinkBtn: function (id) {
      //判断是添加还是编辑
      let name = "addAttractionsPoint";
      if (id) {
        name = "editAttractionsPoint";
      }
      this.$router.push({
        name: name,
        params: { id: id },
    ...
    getList: function () {
      ...
      attractionsPointApi
        ...
        .then((result) => {
          ...
          this.attractionsPointList = result.rows;
